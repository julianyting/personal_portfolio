...
class db_operations():
    ...
    def __init__(self):
        # Make connection. Credentials come from the environment so the
        # password isn't committed; the defaults match the local dev setup.
        self.connection = mysql.connector.connect(
            host= os.environ.get("NBA_DB_HOST", "localhost"),
            user= os.environ.get("NBA_DB_USER", "root"),
            password= os.environ.get("NBA_DB_PASSWORD", "CPSC408!"),
            auth_plugin= 'mysql_native_password',
            database = os.environ.get("NBA_DB_NAME", "NBA")
            )
...
# FUNCTIONS FOR CREATING TABLES AND POPULATING
    ...
    def update_fk_tables(self):
        # One statement per execute(); the connector rejects several at once.
        statements = [
            '''
            ALTER TABLE Coach
    ...
    def reset(self):
        # Children first, so the foreign keys don't block the drops.
        tables = ['PlayerGameStatistics', 'Game', 'Coach', 'Player', 'Team']
        for table in tables:
...
# -----------------------------
    ...
    def get_player_id(self, name):
        ...
        record = (name,)
        result = self.select_query_params(query, record)
        # Callers below test for None, so miss quietly instead of IndexError.
    ...
    def trade_player(self, playername, newteamname):
        ...
        if player_id is None or team_id is None:
            print(f"Error: could not trade '{playername}' to '{newteamname}'.")
            return
        query = '''
        UPDATE Player
        SET TeamID = %s
...
#Julians Code
    ...
    def delete_team(self, team_name):
        # commit() already wraps this in a transaction; the explicit
        # START TRANSACTION/COMMIT made it an illegal multi-statement query.
        query = '''
        DELETE FROM Team
        WHERE Name = %s'''
        self.cursor.execute(query, (team_name,))
    ...
    def get_games_by_team(self, team_name):
        ...
        INNER JOIN Team AS away_team ON Game.AwayTeamID = away_team.TeamID
    ...
    def add_player_statistics(self, game_id, player_id, points, rebounds, assists, blocks, steals, turnovers, minutes_played, fouls):
        query = '''
        INSERT INTO PlayerGameStatistics (GameID, PlayerID, Points, Rebounds, Assists, Blocks, Steals, Turnovers, MinutesPlayed, Fouls)
    ...
    def get_average_statistics_by_player(self, player_name):
        ...
        INNER JOIN Player ON PlayerGameStatistics.PlayerID = Player.PlayerID
        WHERE Player.Name = %s'''
        self.cursor.execute(query, (player_name,))
        return self.cursor.fetchall()
...
#=============================================================
    ...
    def single_attribute(self, query):
        ...
        # Drop every NULL, not just the first — and don't raise when there are none.
        results = [i[0] for i in results if i[0] is not None]
        return results
